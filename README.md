...
Consumption model for 'independent days' (Food Diary data) can be computed with fixed variances for (A) day-to-day variation of positive consumption amounts & (B) between individual variation of mean positive consumptions. These two variances are then not estimated as unknown variance structure in the hierarchical model, but instead they are set values taken simply from sample variances evaluated directly from the diary data. This requires sufficient data for evaluating sample variances over days, and over individual observed means. By this, the consumption model is simplified, while ignoring the uncertainty about the variance structure (and correlation structures).  
