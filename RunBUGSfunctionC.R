  ...
  # This file runs the model version with FFQ data.
...
for(i in 1:nhM){
  ...
  if((input$priorchoice == "sigma_uniform") ){ 
    # then inits for sigma are needed
    if((nhK>0)&(nhM>0)){
      inits <- function(){list(muw=0,sigw=1,mucK=initmucK,sigcK=initsigcK,mucM=initmucM,sigcM=initsigcM,pM=matrix(0.5,nhM,nf),mus0=rep(0,nf),logitp0=rep(0,nf) ) }
      parameters=c("mucK","mucM","sigcK","sigcM","mus0","Ts0","logitp0","muw","sigw","pM","pK")
    }
    if((nhK>0)&(nhM==0)){
      inits <- function(){list(muw=0,sigw=1,mucK=initmucK,sigcK=initsigcK,mus0=rep(0,nf),logitp0=rep(0,nf) ) }  
      parameters=c("mucK","sigcK","mus0","Ts0","logitp0","muw","sigw","pK")
    }
    if((nhK==0)&(nhM>0)){
      inits <- function(){list(muw=0,sigw=1,mucM=initmucM,sigcM=initsigcM,pM=matrix(0.5,nhM,nf),mus0=rep(0,nf),logitp0=rep(0,nf) ) }  
      parameters=c("mucM","sigcM","mus0","Ts0","logitp0","muw","sigw","pM")
    }  
  } # end of prior choice
  
  if((input$priorchoice == "tau_gamma") ){ 
    # then inits for tau are needed
    if((nhK>0)&(nhM>0)){
      inits <- function(){list(muw=0,tauw=1,mucK=initmucK,taucK=initsigcK^(-2),mucM=initmucM,taucM=initsigcM^(-2),pM=matrix(0.5,nhM,nf),mus0=rep(0,nf),logitp0=rep(0,nf) ) }
      parameters=c("mucK","mucM","sigcK","sigcM","mus0","Ts0","logitp0","muw","sigw","pM","pK")
    }
    if((nhK>0)&(nhM==0)){
      inits <- function(){list(muw=0,tauw=1,mucK=initmucK,taucK=initsigcK^(-2),mus0=rep(0,nf),logitp0=rep(0,nf) ) }  
      parameters=c("mucK","sigcK","mus0","Ts0","logitp0","muw","sigw","pK")
    }
    if((nhK==0)&(nhM>0)){
      inits <- function(){list(muw=0,tauw=1,mucM=initmucM,taucM=initsigcM^(-2),pM=matrix(0.5,nhM,nf),mus0=rep(0,nf),logitp0=rep(0,nf) ) }  
      parameters=c("mucM","sigcM","mus0","Ts0","logitp0","muw","sigw","pM")
    }  
  } # end of prior choice
