  ...
  # This file runs the model version with independent consumption days with fixed variance (sample variances from data).
...
data <- append(data,c("osdlogsw1","osdlogsw2"))  
...
if((nhK==0)&(nhM>0)){
  if((nhM==1)&(nf==1)){nexactM0 <- 1}
data <- list("Weight","logcM","nexactM","nf","nr","nd","logsw","usedays")
if(input$priorchoice == "sigma_uniform"){
...
for(i in 1:nhM){
  ...
  if((input$priorchoice == "sigma_uniform")&(between.user.pvar == 1)){ 
    # then inits for sigma are needed,
    # variability of frequencies between users needs inits (logitp)
    if((nhK>0)&(nhM>0)){
          ...
          parameters=c("mucK","mucM","sigcK","sigcM","mus0","logitp0","Tp","muw","sigw","pM","pK")
    }
    if((nhK>0)&(nhM==0)){
          ...
          parameters=c("mucK","sigcK","mus0","logitp0","Tp","muw","sigw","pK")
    }
    if((nhK==0)&(nhM>0)){
          ...
          parameters=c("mucM","sigcM","mus0","logitp0","Tp","muw","sigw","pM")
    }  
  } # end of prior choice
  
  if((input$priorchoice == "tau_gamma")&(between.user.pvar == 1)){ 
  ...
  if((input$priorchoice == "sigma_uniform")&(between.user.pvar == 0) ){  
    # then prior for sigma is needed,
    # no variability of frequencies between users, no inits (logitp)
    if((nhK>0)&(nhM>0)){
    ...
    if((nhK==0)&(nhM>0)){
          ...
          parameters=c("mucM","sigcM","mus0","logitp0","muw","sigw","pM")
    }  
  } # end of prior choice
  
  if((input$priorchoice == "tau_gamma")&(between.user.pvar == 0)){  
