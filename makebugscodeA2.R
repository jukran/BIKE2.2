...
cat("# Consumption code",file="bikemodel.txt",sep="\n",append=TRUE)
  ...
  cat("  ",file="bikemodel.txt",sep="\n",append=TRUE)
    ...
    # create a model where variance between consumers, and variance between servings, is a fixed value (simple estimate from data)
    cat("# Consumption measurements data (daily amounts)",file="bikemodel.txt",sep="\n",append=TRUE)
    
    cat("for(i in 1:nf){ # food",file="bikemodel.txt",sep="\n",append=TRUE)
    cat("otaulogsw2[i] <- pow(osdlogsw2[i],-2)",file="bikemodel.txt",sep="\n",append=TRUE)
    ...
    cat("logsw[r,t,i] ~ dnorm(mus[r,i],otaulogsw2[i])",file="bikemodel.txt",sep="\n",append=TRUE)
    cat("}}}",file="bikemodel.txt",sep="\n",append=TRUE)
    
    cat("for(i in 1:nf){ # food",file="bikemodel.txt",sep="\n",append=TRUE)
    cat("otaulogsw1[i] <- pow(osdlogsw1[i],-2) ",file="bikemodel.txt",sep="\n",append=TRUE)
    cat("for(r in 1:nr){ # individual respondent",file="bikemodel.txt",sep="\n",append=TRUE)
    cat("mus[r,i] ~ dnorm(mus0[i],otaulogsw1[i])",file="bikemodel.txt",sep="\n",append=TRUE)  
    cat("}}",file="bikemodel.txt",sep="\n",append=TRUE)
