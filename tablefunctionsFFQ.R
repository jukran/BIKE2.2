...
# Table functions when FFQ data is modelled

# FFQ data represents reported mean daily consumptions of each food for each respondent (possibly zeros, if truly non-consumer). 
# Hence, the models have not day-to-day variation. Only variation of mean consumptions between individuals.
# Therefore, acute exposures cannot be estimated. Only mean exposures for both chemical and microbiological hazards.
...
## ---- resultValues --------
table1FFQ <- function(n_sim, input_modelchoice,input_modelchoice2,input_modelchoice3,input_modelchoice4,input_modelchoice5,
  ...
  if(is.element("Consumptions",theresults)|is.element("Exposures",theresults)){
    ...
    if(nfused>0){
      ...
      if(nhusedM>0){ # formatting for posterior predictive distributions for microbial hazards (chronic, here FFQ data)
        musmc <- matrix(NA,n_sim,nf)
        wmc <- numeric()
        Umc <- matrix(NA,n_sim,nf)
        EemcM <- array(0,dim=c(n_sim,nhusedM,nfused)) # default = 0
        chronictotM <- matrix(NA,n_sim,nhusedM)
        RM = matrix(NA,nf,nhM)
        ...
        hlo90totchronicM <- numeric(nhusedM)
        hup90totchronicM <- numeric(nhusedM)
        hmediantotchronicM <- numeric(nhusedM)
        hlo80totchronicM <- numeric(nhusedM)
        hup80totchronicM <- numeric(nhusedM)
        hlo98totchronicM <- numeric(nhusedM)
        hup98totchronicM <- numeric(nhusedM)
      } # end of nhusedM
      ...
      if(nhusedK>0){ # simulate posterior predictive distributions for chemical hazards
          ...
          for(mc in 1:(n_sim)){
            ...
            Umc[mc,1:nf] <- rbinom(nf,rep(1,nf),p0[mc,1:nf] )  # actual true (non-)consumer or not
      ...
      if(nhusedM>0){  # simulate posterior predictive distributions for microbial hazards
          ...
          for(mc in 1:(n_sim)){
            ...
            if(nf>1){  # many foods
              musmc[mc,1:nf] <- rmvnorm(1,mus0[mc,1:nf],Ss0[mc,1:nf,1:nf]) # individual mean log-consumption/bw 
            } 
            
            if(nf==1){ # only one food
              musmc[mc,1] <- rnorm(1,mus0[mc,1],sqrt(Ss0[mc,1,1])) # individual mean log-consumption/bw 
            }
            
            wmc[mc] <- rlnorm(1,muw[mc],sigw[mc]) # bodyweight for random individual
            Umc[mc,1:nf] <- rbinom(nf,rep(1,nf),p0[mc,1:nf] ) # actual true (non-)consumer or not
            
            for(h in 1:nhusedM){ #Predict the mean of poisson distribution of bacteria counts: (individual mean exposures)
              for(i in 1:nfused){
                # chronic exposure for a random consumer, hazard h, food i:
                if(nexactM[hazardindexM[h],foodindex[i]]>0){ # hazard-food is modeled
                  EemcM[mc,h,i] <- Umc[mc,foodindex[i]]*pM[mc,hazardindexM[h],foodindex[i]]*PM[foodindex[i],hazardindexM[h]]* 
                    exp(musmc[mc,foodindex[i]])*wmc[mc]*   
                    RM[foodindex[i],hazardindexM[h]]*
                    exp(mucM[mc,hazardindexM[h],foodindex[i]]+0.5*sigcM[mc,hazardindexM[h],foodindex[i]] )
                }
              }
              chronictotM[mc,h] <- sum(EemcM[mc,h,1:nfused]) # sum over foods
      ...
      if(nhusedM>0){
        for(h in 1:nhusedM){  # posterior predictive summaries (quantiles) of individual mean exposures (FFQ data)
          hlo90totchronicM[h] <- round(quantile(chronictotM[,h],0.05,names=FALSE))
          hmediantotchronicM[h] <- round(quantile(chronictotM[,h],0.5,names=FALSE))
          hup90totchronicM[h] <- round(quantile(chronictotM[,h],0.95,names=FALSE))
          hlo80totchronicM[h] <- round(quantile(chronictotM[,h],0.10,names=FALSE))
          hup80totchronicM[h] <- round(quantile(chronictotM[,h],0.90,names=FALSE))
          hlo98totchronicM[h] <- round(quantile(chronictotM[,h],0.01,names=FALSE))
          hup98totchronicM[h] <- round(quantile(chronictotM[,h],0.99,names=FALSE))
        }
      }
      ...
      if(is.element("Consumptions",theresults)){
        ...
        for(i in 1:nfused){ # posterior predictive summaries (quantiles) of individual chronic consumptions (/bw and absolute)
          ...
          fconslo90bw[i] <- quantile(exp(musmc[,foodindex[i]]),0.05,names=FALSE)  ### +0.5*Vs was in all lines below
          fconslo90[i] <- quantile(exp(musmc[,foodindex[i]]+muw+0.5*sigw^2),0.05,names=FALSE)
          fconsup90bw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.95,names=FALSE)
          fconsup90[i] <- quantile(exp(musmc[,foodindex[i]]+muw+0.5*sigw^2),0.95,names=FALSE) 
          fconslo80bw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.10,names=FALSE)
          fconslo80[i] <- quantile(exp(musmc[,foodindex[i]]+muw+0.5*sigw^2),0.10,names=FALSE)
          fconsup80bw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.90,names=FALSE)
          fconsup80[i] <- quantile(exp(musmc[,foodindex[i]]+muw +0.5*sigw^2),0.90,names=FALSE)
          fconslo98bw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.01,names=FALSE) 
          fconslo98[i] <- quantile(exp(musmc[,foodindex[i]]+muw +0.5*sigw^2),0.01,names=FALSE)
          fconsup98bw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.99,names=FALSE) 
          fconsup98[i] <- quantile(exp(musmc[,foodindex[i]]+muw +0.5*sigw^2),0.99,names=FALSE)
          fconsmedianbw[i] <- quantile(exp(musmc[,foodindex[i]] ),0.5,names=FALSE)
          fconsmedian[i] <- quantile(exp(musmc[,foodindex[i]]+muw +0.5*sigw^2),0.5,names=FALSE)
        } # end of nfused
      ...
      if(nhusedM>0){
        DF1M <- data.frame(
          ...
          Median = as.character(round(hmediantotchronicM[1:nhusedM],2)),
...
## ---- resultProbs --------
table2FFQ <- function(n_sim, input_modelchoice,input_modelchoice2,foodnamesused,nfused,foodindex,hazardnames,
      ...
      for(i in 1:nfused){
        if(nexactK[hazardindexK[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          for(u in 1:(n_sim)){ 
            ...
            # P(exposure < limit) over variability dimension:
          ...
          qu95_95 <- round(quantile(qlnorm(0.95,logRK[foodindex[i],hazardindexK[h]]
                                           +mus0[,foodindex[i]]
                                           +mucK[,hazardindexK[h],foodindex[i]]
                                           +0.5*sigcK[,hazardindexK[h],foodindex[i]]^2,
                                           sqrt(Vs0) ),
                                    0.95,names=FALSE,na.rm=TRUE),3)
          qu95_05 <- round(quantile(qlnorm(0.95,logRK[foodindex[i],hazardindexK[h]]
                                           +mus0[,foodindex[i]]
                                           +mucK[,hazardindexK[h],foodindex[i]]
                                           +0.5*sigcK[,hazardindexK[h],foodindex[i]]^2,
                                           sqrt(Vs0) ),
      ...
      for(i in 1:nfused){
        if(nexactM[hazardindexM[h],foodindex[i]]>0){ # this hazard-food is modeled
          ...
          for(u in 1:(n_sim)){ 
            ...
            # evaluate the 95% quantile of the mean exposure (for microbiological hazard) distribution including all days 
            ...
            punderlimitallM[u] <- 
              ...
              plnormM 
            # contaminated consumptions: (one possible case: F(limit) cumulative prob.)
            # from lnorm-variability of mean microbial exposures 
          ...
          DFplimposM <- data.frame(Quantity = paste0("P(chronic <",limitexpoM[h], ")"),
                                   ...
                                   #Quantity = paste0("P(chronic ",hazardnamesusedM[h]," from pos days, ",foodnamesused[i],", <",limitexpoM[h],")"),
          ...
          DFplimallM <- data.frame(Quantity = paste0("P(chronic<",limitexpoM[h], ")"),
                                   ...
                                   Food = paste0(foodnamesused[i]),
                                   # Limit = paste0("<",limitexpoM[h]),
                                   #Quantity = paste0("P(chronic ",hazardnamesusedM[h]," from all days, ",foodnamesused[i],", <",limitexpoM[h],")"),
          ...
          DF95allM <- data.frame(Quantity = paste0("Q95(chronic)"), 
                                 From = paste0("all days"),
                                 Hazard = paste0(hazardnamesusedM[h]),
                                 Food = paste0(foodnamesused[i]),
                                 # Limit = paste0(""),
                                 #Quantity = paste0("Q95 chronic ",hazardnamesusedM[h]," from all days, ",foodnamesused[i]), 
                                 Q05 = as.character(round(quantile(qutotal95,0.05,names=FALSE,na.rm=TRUE),2)),
                                 Q50 = as.character(round(quantile(qutotal95,0.5,names=FALSE,na.rm=TRUE),2)),
                                 Q95 = as.character(round(quantile(qutotal95,0.95,names=FALSE,na.rm=TRUE),2)), 
                                 stringsAsFactors=FALSE)
          ...
          DF95posM <- data.frame(Quantity = paste0("Q95(chronic)"), 
                                 From = paste0("pos days"),
                                 Hazard = paste0(hazardnamesusedM[h]),
                                 Food = paste0(foodnamesused[i]),
                                 # Limit = paste0(""),
                                 #Quantity = paste0("Q95 chronic ",hazardnamesusedM[h]," from pos days, ",foodnamesused[i]), 
                                 Q05 = as.character(round(qu95_05,2)),
                                 Q50 = as.character(round(qu95_50,2)),
                                 Q95 = as.character(round(qu95_95,2)), 
                                 stringsAsFactors=FALSE)
